...
public class NavNodeEditor : MonoBehaviour
{
    ...
    private Nav_node navNode = null;
    private Nav_node activeNavNode = null;
    ...
    void OnScene(SceneView scene)
    {
        ...
        if (e.isMouse && (e.type == EventType.MouseMove || e.type == EventType.MouseDrag))
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hitInfo, 100, layerMask))
            {
                ...
                spawnable = !(hitInfo.collider.gameObject.TryGetComponent<Nav_node>(out navNode));
                e.Use();
            }
            else
            {
        ...
        if (e.isKey && e.keyCode == KeyCode.D)
        {
            if (navNode != null)
            {
                ...
                foreach (Nav_node neighbor in navNode.neighbors)
                {
    ...
    private void OnDrawGizmos()
    {
        ...
        if (activeNavNode != null)
        {
            ...
            foreach (Nav_node neighbor in activeNavNode.neighbors)
            {
                if (neighbor == navNode)
                {
                    connected = true;
        ...
        foreach (Nav_node node in nodes)
        {
            foreach (Nav_node neighbors in node.neighbors)
            {
